...
description: 'Build, redesign, implement, review, or improve any user-facing interface with context-appropriate visual quality and production-ready interaction. Use for websites, landing pages, dashboards, admin or CRM tools, commerce, content products, mobile or desktop apps, games, editors, forms, responsive UI, accessibility, design-system work, Figma or screenshot implementation, and requests for beautiful, polished, modern, distinctive, usable, or production-ready UI. Enforces good UI rather than one fixed style: user and task fit, hierarchy, complete states, visual coherence, real content and assets, accessibility, responsive behavior, and rendered verification. Do not use for backend-only, API-only, database-only, CLI-only, or pure image and logo tasks with no interface.'
---
